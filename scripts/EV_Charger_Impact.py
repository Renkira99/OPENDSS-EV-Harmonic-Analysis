...
def collect_results(case_name):
    ...
    results['total_losses_kw'] = losses[0] / 1000
    results['total_losses_kvar'] = losses[1] / 1000
    ...
    for i in range(dss.Circuit.NumBuses()):
        ...
        if len(pu_voltages) > 0:
            ...
            avg_voltage = np.mean([v for v in magnitudes if v > 0.01])
    ...
    while flag > 0:
        ...
        if len(powers) >= 6:
            ...
            for j in range(0, 6, 2):
                p = powers[j]
                q = powers[j+1] if j+1 < len(powers) else 0
                s_total += np.sqrt(p**2 + q**2)
            
            loading_pct = (s_total / kva_rating) * 100 if kva_rating > 0 else 0
...
def compare_results(base_results, ev_results):
    ...
    print("\n📊 SYSTEM SUMMARY")
    print("-"*70)
    print(f"{'Metric':<35} {'Base Case':>15} {'With EV':>15} {'Change':>15}")
    print("-"*70)
    ...
    if ev_results['buses_under_voltage']:
        ...
        for bus, voltage in sorted(ev_results['buses_under_voltage'], key=lambda x: x[1])[:10]:
            print(f"    {bus}: {voltage:.4f} p.u.")
...
def plot_voltage_comparison(base_results, ev_results, ev_chargers):
    ...
    ax1 = axes[0]
    base_voltages = [v['voltage_pu'] for v in base_results['bus_voltages'] if 0.8 < v['voltage_pu'] < 1.1]
    ev_voltages = [v['voltage_pu'] for v in ev_results['bus_voltages'] if 0.8 < v['voltage_pu'] < 1.1]
    
    ax1.hist(base_voltages, bins=30, alpha=0.6, label='Base Case', color='blue', edgecolor='black')
    ax1.hist(ev_voltages, bins=30, alpha=0.6, label='With EV Chargers', color='red', edgecolor='black')
    ax1.axvline(x=0.95, color='orange', linestyle='--', linewidth=2, label='Lower Limit (0.95 pu)')
    ax1.axvline(x=1.05, color='orange', linestyle='--', linewidth=2, label='Upper Limit (1.05 pu)')
    ax1.set_xlabel('Voltage (p.u.)')
    ax1.set_ylabel('Number of Buses')
    ax1.set_title('Bus Voltage Distribution')
    ax1.legend()
